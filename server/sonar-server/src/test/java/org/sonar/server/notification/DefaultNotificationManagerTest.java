...
public class DefaultNotificationManagerTest {
  ...
  public void shouldFindSubscribedRecipientForGivenResource() {
    ...
    when(authorizationDao.keepAuthorizedLoginsOnProject(any(), eq(Arrays.asList("user1", "user3")), eq("uuid_45"), eq("user")))
      .thenReturn(Sets.newHashSet("user1", "user3"));
    when(authorizationDao.keepAuthorizedLoginsOnProject(any(), eq(Arrays.asList("user3")), eq("uuid_45"), eq("user")))
      .thenReturn(Sets.newHashSet("user3"));
