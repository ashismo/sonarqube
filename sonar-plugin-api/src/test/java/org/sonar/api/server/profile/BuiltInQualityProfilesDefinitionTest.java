  ...
  public void createProfileWithRules() {
    ...
    assertThat(profile.rules())
      .extracting(BuiltInQualityProfilesDefinition.BuiltInActiveRule::repoKey, BuiltInQualityProfilesDefinition.BuiltInActiveRule::ruleKey,
        BuiltInQualityProfilesDefinition.BuiltInActiveRule::overridenSeverity, r -> r.overridenParams().size())
      .containsOnly(
        tuple("repo", "ruleWithoutParam", null, 0),
        tuple("repo", "ruleWithSeverity", "CRITICAL", 0),
        tuple("repo", "ruleWithParam", null, 1));
    assertThat(profile.rule(RuleKey.of("repo", "ruleWithParam")).overridenParam("param").key()).isEqualTo("param");
    assertThat(profile.rule(RuleKey.of("repo", "ruleWithParam")).overridenParam("param").overridenValue()).isEqualTo("value");
  }
